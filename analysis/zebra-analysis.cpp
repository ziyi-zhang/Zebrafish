...
bool compute_analysis(
  ...
  for (int sim = fStart; sim <= fEnd; ++sim) {
    // HERE/////////////////////////////////////////////////////////////////////
    //  const json args = {
    //      {"tensor_formulation", is_linear ? "LinearElasticity" :
    //      "NeoHookean"},
    //      {"discr_order", discr_order},
    //      {"params",
    //       {{"E", E / double(1e6)},
    //        {"nu", nu}}}, // E has unit [Pascal], convert that to [um] unit
    //      {"problem", "PointBasedTensor"}};

    // const Eigen::MatrixXd currentv = V[sim].block(0, 0, bm_offset_v, 3) -
    // bm_v;

    ///////HERE/////////////////////////////////////////////////////////////////////
    // state.init(args);
    // polyfem::PointBasedTensorProblem &tproblem =
    //     *dynamic_cast<polyfem::PointBasedTensorProblem
    //     *>(state.problem.get());
    // tproblem.add_constant(1, zero);
    // Eigen::Matrix<bool, 3, 1> dirichet_dims;
    // dirichet_dims.setConstant(true);
    // tproblem.add_function(2, currentv, bm_v, rbf_function, eps, -1,
    //                       dirichet_dims);
    // state.solve();

    // // output
    // const auto OutputHelper = [&state, &belowCage,
    //                            &aboveCage](const std::string &out_path,
    ...
    //   state.interpolate_boundary_function_at_vertices(mesh_v, mesh_f,
    //   state.sol,
    //                                                   displacement_vec);
    //   // guaranteed that (belowCage || aboveCage) == True
    //   if (belowCage)
    //     state.interpolate_boundary_tensor_function(
    //         mesh_v, mesh_f, state.sol, displacement_vec, true,
    //         traction_forces, stress, mises);
    //   if (aboveCage)
    //     state.interpolate_boundary_tensor_function(
    //         mesh_v, mesh_f_flip, state.sol, displacement_vec, true,
    //         traction_forces_flip, stress_flip, mises_flip);

    //   // per-triangle data -> per-vertex data by taking average
    //   const auto ToVertexData =
    //       [&mesh_v, &mesh_f](const Eigen::MatrixXd &traction_forces_tri,
    ...
    //         stress_ver = Eigen::MatrixXd::Zero(mesh_v.rows(), 9); // 3x3
    //         matrix mises_ver = Eigen::MatrixXd::Zero(mesh_v.rows(), 1);
    ...
    //           for (int d = 0; d < 3; ++d) {
    //             int vid = mesh_f(f, d);
    //             traction_forces_ver.row(vid) +=
    //                 traction_forces_tri.row(f) * area(f);
    ...
    //   // triangle to vertex
    //   Eigen::MatrixXd traction_force_v, traction_force_v_flip,
    //       traction_force_res;
    //   Eigen::MatrixXd stress_v, stress_v_flip, stress_res;
    //   Eigen::MatrixXd mises_v, mises_v_flip, mises_res;
    //   if (belowCage)
    //     ToVertexData(traction_forces, stress, mises, traction_force_v,
    //     stress_v,
    //                  mises_v);
    //   if (aboveCage)
    //     ToVertexData(traction_forces_flip, stress_flip, mises_flip,
    //                  traction_force_v_flip, stress_v_flip, mises_v_flip);
    //   std::string suffix = "";
    ...
    //   } else if (aboveCage && belowCage) {
    //     traction_force_res =
    //         (traction_force_v_flip + traction_force_v).array() / 2.0;
    //     stress_res = (stress_v_flip + stress_v).array() / 2.0;
    //     mises_res = (mises_v_flip + mises_v).array() / 2.0;
    //     suffix = "_avg";
    //   } else {
    //     std::cerr << "neither aboveCage or belowCage? impossible!" <<
    //     std::endl; return;
    //   }
    ...
    //   for (int i = 0; i < 3; ++i) {
    //     for (int j = 0; j < 3; ++j) {
    //       const std::string index = "_" + std::to_string(i) +
    //       std::to_string(j); VTUwriter.add_field("stress" + suffix + index,
    //                           stress_res.col(i * 3 + j));
    //     }
    //   }
    ...
    const auto RemoveOneRing = [&result_f](Eigen::MatrixXi &new_f) {
      ...
      for (int i = 0; i < result_f.rows(); i++) {
        if (std::find(boundary_v.begin(), boundary_v.end(), result_f(i, 0)) !=
                boundary_v.end() ||
            std::find(boundary_v.begin(), boundary_v.end(), result_f(i, 1)) !=
                boundary_v.end() ||
            std::find(boundary_v.begin(), boundary_v.end(), result_f(i, 2)) !=
                boundary_v.end()) {
