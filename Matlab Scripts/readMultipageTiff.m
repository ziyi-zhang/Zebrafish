...
% Ziyi. Feb, 2020.
    ...
    channels = extractNumber(description, 'channels=');
    slices = extractNumber(description, 'slices=');
    frames = extractNumber(description, 'frames=');
    if (totalImages<0 || channels<0 || slices<0 || frames<0)
        warning("Failure on interpreting image description.");
        return;
    end
    assert(totalImages == channels * slices * frames);
    meta = zeros(3, totalImages); % channel, slice, frame
    count = 0;
    for i = 1:frames
        for j = 1:slices
            for k = 1:channels
    ...
    fprintf('Channels=%d, Slices=%d, Frames=%d\n', channels, slices, frames);
