...
def Carve(V, F, V_shell_out):
    ...
    for i in range(0, N):
        ...
        pt2 = Point(V[F[i, 1], 0], V[F[i, 1], 1])
        pt3 = Point(V[F[i, 2], 0], V[F[i, 2], 1])
        if not (pt1.within(poly) and pt2.within(poly) and pt3.within(poly)):
...
def Point2Area(pt, poly_in, poly_out, i, count):
    if pt.within(poly_in):
        count[i, 0] += 1
    elif pt.within(poly_out):
        count[i, 1] += 1
    else:
        count[i, 2] += 1
    return
...
def CategorizeTriangle(V, F, V_shell_in, V_shell_out):

    N = F.shape[0]
    count = np.full((N, 3), 0, dtype=int)
    cat = np.full((N), 0, dtype=int)
    poly_in = Polygon(V_shell_in)
    poly_out = Polygon(V_shell_out)
    ...
    for i in range(0, N):
        ...
        pt2 = Point(V[F[i, 1], 0], V[F[i, 1], 1])
        pt3 = Point(V[F[i, 2], 0], V[F[i, 2], 1])
        Point2Area(pt1, poly_in, poly_out, i, count)
        Point2Area(pt2, poly_in, poly_out, i, count)
        Point2Area(pt3, poly_in, poly_out, i, count)
        # which type is this triangle?
        assert(sum(count[i, :]) == 3)
        if (count[i, 2] == 3):
            cat[i] = 1  # completely outside
        elif (count[i, 0] == 0 and count[i, 1] > 0 and count[i, 2] > 0):
            cat[i] = 2  # outside & middle
        elif (count[i, 0] == 1 and count[i, 1] == 1 and count[i, 2] == 1):
            cat[i] = 3  # outside & middle & inside
        elif (count[i, 2] == 0 and count[i, 0] > 0 and count[i, 1] > 0):
            cat[i] = 4  # middle & inside
        elif (count[i, 0] == 3):
            cat[i] = 5  # completely inside
        elif (count[i, 1] == 3):
            cat[i] = 6  # middle
        else:
            assert(False)
