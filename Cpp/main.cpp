...
bool ValidStartingPoint(const image_t &image, const bspline &bsp, double x,
                        double y, double z, double r) {

  static const double thres = QuantileImage(image, 0.85);

  // valid cylinder
  if (!cylinder::IsValid(bsp, x, y, z, r, 3.0))
    return false;
  // membrane
...
int main(int argc, char **argv) {
  ...
  unsigned int num_threads =
      32; // std::max(1u, std::thread::hardware_concurrency());
  int lsMethod = 2;
  int layerPerImg;
  CLI::App command_line{"ZebraFish"};
  command_line
      .add_option("-i,--img", image_path, "Input TIFF image to process")
  ...
  command_line.add_option(
      "-h", layerPerImg, "Input the number of layers (slices) in each 3D scan");
  ...
  cout << "Each layer clipped to be " << image[0].rows() << " x "
       << image[0].cols() << endl;
  ...
  cout << "Image normalized: most pixels will have value between 0 and 1"
       << endl;
    ...
    for (int iy = 0; iy < floor(Ny / gapY); iy++)
      for (int iz = 0; iz < floor(Nz / gapZ); iz++)
        for (int ir = 0; ir < Nr; ir++) {
          ...
          zz = iz * gapZ;
          rr = rArray[ir];
          if (!ValidStartingPoint(image, bsplineSolver, xx, yy, zz, rr))
            continue;
  ...
  tbb::parallel_for(tbb::blocked_range<int>(0, sampleCount),
                    [&sampleInput, &sampleOutput,
                     &bsplineSolver](const tbb::blocked_range<int> &r) {
                      for (int ii = r.begin(); ii != r.end(); ++ii) {
                        cylinder::EvaluateCylinder(
                            bsplineSolver, sampleInput(ii, 0),
                            sampleInput(ii, 1), sampleInput(ii, 2),
                            sampleInput(ii, 3), 3, sampleOutput(ii));
                      }
                    });
  ...
  sampleOutput_Newton.resize(sampleCountNewton,
                             6); // x, y, z, r, energy, iteration
  logger().info("Optimization #starting points = {}", sampleCountNewton);

  // Optimization
  logger().info("Before optimization");
  tbb::parallel_for(
      tbb::blocked_range<int>(0, sampleCountNewton),
      //////////////////////////////////////
      // lambda function for parallel_for //
      //////////////////////////////////////
      [&sampleInput_Newton, &sampleOutput_Newton, &bsplineSolver,
       &param](const tbb::blocked_range<int> &r) {
        ...
        for (int ii = r.begin(); ii != r.end(); ++ii) {
          ...
          ///////////////////////////////////
          auto func = [&sampleInput_Newton, &bsplineSolver,
                       ii](const VectorXd &x, VectorXd &grad) {
            DScalar ans;

            if (!cylinder::IsValid(bsplineSolver, x(0), x(1),
                                   sampleInput_Newton(ii, 2), x(2), 3)) {
              grad.setZero();
              return 1.0;
            }
            cylinder::EvaluateCylinder(
                bsplineSolver, DScalar(0, x(0)), DScalar(1, x(1)),
                sampleInput_Newton(ii, 2), DScalar(2, x(2)), 3, ans);
            grad.resize(3, 1);
  ...
  for (int i = 0; i < sampleCountNewton; i++) {
    ...
    printf("%.3f %.3f %.1f %.3f %f %.0f   %.3f %.3f %.1f %.3f\n",
           sampleOutput_Newton(i, 0), sampleOutput_Newton(i, 1),
           sampleOutput_Newton(i, 2), sampleOutput_Newton(i, 3),
           sampleOutput_Newton(i, 4), sampleOutput_Newton(i, 5),
           sampleInput_Newton(i, 0), sampleInput_Newton(i, 1),
