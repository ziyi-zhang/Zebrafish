...
void GUI::DrawStage4() {
  ...
  if (showOptimizedPoints) {
    ...
    if (optimPointLoc.rows() > 0) {
      ...
      viewer.data().add_points(optimPointLoc, pointColor);
    }
  ...
  if (ImGui::CollapsingHeader("Optimization", ImGuiTreeNodeFlags_DefaultOpen)) {
    ...
    if (ImGui::TreeNode("Advanced optimization")) {
      ...
      ImGui::PlotHistogram("", optimEnergyHist.hist.data(),
                           optimEnergyHist.hist.size(), 0, NULL, 0,
                           optimEnergyHist.hist.maxCoeff(), ImVec2(0, 80));
    ...
    if (showTooltip && ImGui::IsItemHovered()) {
      ImGui::SetTooltip("Start optimization with the specified precision. This "
                        "may take some time.");
    }
    ImGui::SameLine();
...
void GUI::Optimization() {
  ...
  tbb::parallel_for(
      ...
      //////////////////////////////////////
      [this /*.grid_search, .optimization, &bsplineArray[0]*/,
       &param](const tbb::blocked_range<int> &r) {
        ...
        for (int ii = r.begin(); ii != r.end(); ++ii) {
          ...
          ///////////////////////////////////
          auto func = [this /*.grid_search, .bsplineArray[0]*/,
                       ii](const Eigen::VectorXd &x, Eigen::VectorXd &grad) {
            DScalar ans;

            if (!cylinder::IsValid(bsplineArray[0], x(0), x(1),
                                   pointRecord.grid_search(ii, 2), x(2),
                                   cylinder::H)) {
              grad.setZero();
              return 1.0;
            }
            cylinder::EvaluateCylinder(
                bsplineArray[0], DScalar(0, x(0)), DScalar(1, x(1)),
                pointRecord.grid_search(ii, 2), DScalar(2, x(2)), cylinder::H,
                ans, invertColor);
            grad.resize(3, 1);
...
void GUI::UpdateOptimPointLoc() {
  ...
  optimPointLoc.col(1) =
      (imgRows - 0.5) - pointRecord.optimization.col(0).array();
  optimPointLoc.col(2) = pointRecord.optimization.col(2);

  logger().info("   [Visualization] Optimization resultant points updated: "
                "total number = {}",
                N);
}
...
void GUI::UpdateOptimEnergyHist() {
  ...
  tmp.setZero();
  for (int i = 0; i < N; i++) {
    tmp[std::floor((energyCol(i) - minValue) / gap)]++;
  }
  optimEnergyHist.hist = tmp.cast<float>();
}
