    ...
    tbb::parallel_for( tbb::blocked_range<int>(0, N),
        ...
        for (int ii = r.begin(); ii != r.end(); ++ii) {    
                ...
                (const Eigen::VectorXd& x, Eigen::VectorXd& grad) {
                        ...
                        if (!cylinder::IsValid(bsplineArray[0], x(0), x(1), pointRecord.grid_search(ii, 2), x(2), cylinder::H)) {
                            grad.setZero();
                            return 1.0;
                        }
                        cylinder::EvaluateCylinder(bsplineArray[0], DScalar(0, x(0)), DScalar(1, x(1)), pointRecord.grid_search(ii, 2), DScalar(2, x(2)), cylinder::H, ans, reverseColor);
                        grad.resize(3, 1);
