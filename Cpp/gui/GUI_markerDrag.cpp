...
void GUI::RenderMarkerDragGUI() {
  ...
  if (ImGui::TreeNode("Manually move markers")) {
    ...
    if (showTooltip && ImGui::IsItemHovered()) {
      ImGui::SetTooltip(
          "Manually change the XY location of the markers using mouse.\nCheck "
          "this box and move close to a marker. Click and hold to select one "
          "marker. Drag it to a new location and release button.");
    }
    ...
    if (showTooltip && ImGui::IsItemHovered()) {
      ImGui::SetTooltip(
          "Optimize the XY location of markers in this frame again. This is "
          "optional.\nIf any marker has been manually moved, it is highly "
          "suggested to optimize this frame again.\nThis button does not "
          "optimize the depth.");
    }

    static int currentSelectedMarker = 0;
    static double deltaZ = 0.0;
    const Eigen::MatrixXd &loc = markerPointLocArray[frameToShow];
    const int N = loc.rows();

    ImGui::Separator();
    ImGui::Text("Move marker on z");
    ImGui::SliderInt("Marker", &currentSelectedMarker, 0, N - 1);

    ImGui::InputDouble("Amount", &deltaZ, 0.0, 0.0, "%.2f");
    if (ImGui::Button("Move z")) {
      // override the location
      markerRecord_t &markerRecord = markerArray[frameToShow];
      if (currentSelectedMarker < 0 ||
          currentSelectedMarker > markerRecord.num - 1) {
        logger().error("[Fatal error] currentSelectedMarker is invalid: {}",
                       currentSelectedMarker);
      } else {
        markerRecord.loc(currentSelectedMarker, 2) += deltaZ;
        UpdateMarkerPointLocArray();
        logger().debug("Manually move marker: index {} moved by {}",
                       currentSelectedMarker, deltaZ);
      }
    }
...
void GUI::MouseSelectMarker(const Eigen::Vector2f &mouse) {
  /// called by "MouseMoveCallback" when "markerDragActive" is true and
  /// "markerDragFocused" is false
  ...
  if (hit) {
    ...
    if (!markerDragFocused) {
      ...
      for (int i = 0; i < markerRecord.num; i++) {
        ...
        dist_square =
            (markerRecord.loc(i, 0) - x) * (markerRecord.loc(i, 0) - x) +
            (markerRecord.loc(i, 1) - y) * (markerRecord.loc(i, 1) - y);
...
void GUI::MarkerDragSetNewLoc() {
  ...
  if (markerDragHitIndex < 0 || markerDragHitIndex > markerRecord.num - 1) {
    logger().error("[Fatal error] markerDragHitIndex is invalid: {}",
                   markerDragHitIndex);
  } else {
    markerRecord.loc(markerDragHitIndex, 0) = x;
    markerRecord.loc(markerDragHitIndex, 1) = y;
    logger().debug("Manually move marker: index {} moved to location {}x{}",
                   markerDragHitIndex, x, y);
  }
...
void GUI::MarkerDragVisualization() {
  ...
  if (markerDragHit) {
    ...
    selectedLoc(2) =
        markerPointLocArray[frameToShow](markerDragHitIndex, 2) + deltaZ;
