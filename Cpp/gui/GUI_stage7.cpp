...
void GUI::DrawStage7() {
  ...
  if (preLoadAllFrames) {
    ...
    if (ImGui::CollapsingHeader("Load Frames",
      ...
      if (showTooltip && ImGui::IsItemHovered()) {
        ImGui::SetTooltip("Load desired number of frames and prepare them for "
                          "optimization. This may take some time.");
      }
  ...
  if (ImGui::CollapsingHeader("Optical Flow", ImGuiTreeNodeFlags_DefaultOpen)) {
    ...
    if (showTooltip && ImGui::IsItemHovered()) {
      ImGui::SetTooltip(
          "Weighting factor in Horn Schunck optical flow. The square of this "
          "value will be used in the energy function.\nLarger weighting factor "
          "will make the flow field smoother.");
    }
    ...
    if (showTooltip && ImGui::IsItemHovered()) {
      ImGui::SetTooltip(
          "The number of iterations used when solving optical flow.\nA large "
          "number will make optical flow correction more accurate.\nA small "
          "number will make the computation faster.");
    }
    ...
    if (ImGui::Button("Start Optical Flow")) {
      try {
        ...
        std::cerr << "   <button> [Optical Flow] Fatal error encountered."
                  << std::endl;
    ...
    if (showTooltip && ImGui::IsItemHovered()) {
      ImGui::SetTooltip(
          "Run optical flow. This may take some time.\nNote optical flow is "
          "**optional**. It makes the pipeline more stable, but it is OK to "
          "proceed without optical flow if the displacement is small.");
    }
    ImGui::SameLine();
...
void GUI::LoadSubsequentFrames() {
  ...
  ReadImage(imagePath, layerPerImg, channelVec, desiredFrames, imgData,
            imageCrop.r0, imageCrop.c0, imageCrop.r1, imageCrop.c1);
  currentLoadedFrames = desiredFrames;

  // quantile curtail
  for (int i = 0; i < currentLoadedFrames; i++) {
    double normalizeQuantileRes =
        QuantileImage(imgData[i], normalizeQuantile, layerBegin, layerEnd);
...
void GUI::ComputeBsplineForAllFrames() {
  ...
  //     [this/*.bsplineArray[ii], .imgData[ii]*/, bsplineDegree](const
  //     tbb::blocked_range<int> &r) {

  //         for (int ii = r.begin(); ii != r.end(); ++ii) {
  //             bsplineArray[ii].CalcControlPts(imgData[ii], 0.7, 0.7, 0.7,
  //             bsplineDegree); std::cout << "B-spline computed for frame " <<
  //             ii << std::endl;
  //         }
  //     });
  // logger().info("<<<<<<<<<< After B-spline <<<<<<<<<<");

  // DO NOT GO PARALLEL HERE: it could take too much memory and lead to
  // crash!!!!
  for (int i = 1; i < currentLoadedFrames; i++) {
  ...
  double res;
  for (int i=0; i<currentLoadedFrames; i++) {
      cylinder::EvaluateCylinder(bsplineArray[i], markerArray[0].loc(0, 0),
  markerArray[0].loc(0, 1), markerArray[0].loc(0, 2), markerArray[0].loc(0, 3),
  cylinder::H, res, invertColor); logger().debug("frame {} res = {}", i, res);
  }
  */
}
...
void GUI::RunOpticalFlow() {
  ...
  /// note: "opticalFlowCorrection" vector has been resized before calling this
  /// function

  for (prevFrameIdx = 0; prevFrameIdx < currentLoadedFrames - 1;
       prevFrameIdx++) {

    OpticalFlow::RunOpticalFlow(imgData[prevFrameIdx],
                                imgData[prevFrameIdx + 1], opticalFlowAlpha,
                                opticalFlowIter, ux, uy, uz);
    ...
    // DEBUG PURPOSE
    std::cout << "Optical Flow from [frame " << prevFrameIdx << "] to [frame "
              << prevFrameIdx + 1 << "] " << std::endl;
