...
typedef struct crop_t {
  ...
  crop_t()
      : cropActive(false), downClicked(false), showCropArea(true), r0(-1),
        c0(-1), r1(-1), c1(-1) {
...
typedef struct grid_t {
  ...
  bool skipMembrane;
  double membraneThres; // brightness threshold to decide whether a point is in
                        // the membrane area

  float energyThres; // energy threshold to decide whether a starting point is
                     // worth optimizing

  grid_t()
      : gapX(1.0), gapY(1.0), gapZ(1.0), rArrayMin(3.0), rArrayMax(6.0),
        rArrayGap(1.0), skipMembrane(false), membraneThres(),
        energyThres(-0.1) {}
} grid_t;
...
typedef struct ICP_t {
  ...
  ICP_t()
      : patternRows(0), patternCols(0), patternSpacing(0.0), xDisp(0.0),
        yDisp(0.0), angleRot(0.0), scale(1.0), xStretch(0.0), yStretch(0.0),
        xdistort(0.0), ydistort(0.0) {
...
typedef struct analysis_t {
  ...
  RCMap_t markerRCMap; // map marker index in "markerMeshArray" to (row, col) in
...
private:
  ...
  int channelToLoad;
  int layerBegin, layerEnd; // only slices in this interval in each 3D image
  ...
  std::string maskPath;
  std::string analysisInputPath; // used to decide whether in re-analysis mode
                                 // (empty or not)
  // Hist
  hist_t imgHist;
  ...
  //////////////////////////////////////////////////
  // Optimization
  float optimEnergyThres; // only cylinders with energy smaller than this value
                          // would have the chance to go to "filter" stage
  ...
  // 3D image viewer
  Eigen::MatrixXd
      V_texture; // used to draw imagee (texture) & compute projectiong
  ...
  bool rejectActive;
  bool rejectHit; // checked when "MOUSEMOVE", return true if there is a hit
                  // with clusters
  int rejectMode;
  ...
  Eigen::Matrix<bool, Eigen::Dynamic, 1>
      markerPointStatusArray; // visualization purpose
...
public:
  ...
  GUI();
  void init(std::string imagePath, std::string maskPath,
            std::string analysisInputPath, int debugMode, bool NoGUI);

protected:
  void draw_menu() override;
  void post_resize(int w, int h) override;
  bool MouseDownCallback(igl::opengl::glfw::Viewer &viewer, int button,
                         int modifier);
  bool MouseUpCallback(igl::opengl::glfw::Viewer &viewer, int button,
                       int modifier);
  bool MouseMoveCallback(igl::opengl::glfw::Viewer &viewer, int mouse_x,
                         int mouse_y);
...
private:
  ...
  bool MarkerRecursiveDepthCorrection(int frameIdx, int depthNum,
                                      double depthGap, bool logEnergy = false,
                                      bool forceSecondRound = false,
                                      bool showSuccess = false);
  bool MarkerDepthCorrection(int frameIdx, int num, double gap,
                             bool logEnergy = false, bool showSuccess = false);
  void NormalizeImage(image_t &image, double thres);
  void LoadPreviewImage(std::string path);
  void StateChangeReset();

  // visualization
  void DrawText(Eigen::Vector3d pos, const std::string &text,
                const Eigen::Vector4f color); // DO NOT USE THIS
  ...
  // Stage 3 Grid Search
  void GridSearch();
  bool InMembraneArea(const image_t &image, const double thres, double x,
                      double y, double z, double r);
  bool ValidGridSearchPoint(const image_t &image, const bspline &bsp,
                            bool skipMembrane, double membraneThres, double x,
                            double y, double z, double r);
  void UpdateSampleNewton(const Eigen::MatrixXd &gridSampleInput,
  ...
  void EstimateGlobalMovement(int frame,
  ...
  bool SaveMeshToVTU_point(bool onlySaveFirstFrameMesh,
  ...
  bool SaveImageToTIFF(bool saveMarkerImage, bool saveCellImage,
                       int cellChannel);
  void SaveMeshToOBJ();
