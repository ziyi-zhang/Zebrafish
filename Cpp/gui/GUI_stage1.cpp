...
void GUI::DrawStage1() {
  ...
  if (ImGui::CollapsingHeader("Tiff Image Info",
    ...
    if (showTooltip && ImGui::IsItemHovered()) {
      ImGui::SetTooltip("The number of layers in one frame.\nFor example there "
                        "can be ten frames, and each frame is consisted of 40 "
                        "2D images of size 1024x1024.");
    }
    ImGui::InputInt("Channels Per Slice", &channelPerSlice);
    if (showTooltip && ImGui::IsItemHovered()) {
      ImGui::SetTooltip("The number of channels this image has.\nZebrafish "
                        "will only use the data from one channel, but this "
                        "information is required to properly load the image.");
    }
    ...
    if (ImGui::TreeNode("Advanced channel")) {
      ...
      ImGui::SliderInt("Which channel to load?", &channelToLoad, 0,
                       channelPerSlice - 1, "channel %d");
      if (showTooltip && ImGui::IsItemHovered()) {
  ...
  if (showTooltip && ImGui::IsItemHovered()) {
    ImGui::SetTooltip("Crop the 3D images for all frames in depth.\nOnly "
                      "slices in this closed interval will be used.");
  }
  ...
  if (ImGui::CollapsingHeader("Area Crop", ImGuiTreeNodeFlags_DefaultOpen)) {
    ...
    if (showTooltip && ImGui::IsItemHovered()) {
      ImGui::SetTooltip(
          "Use mouse to select an area of interest.\nClick this once and move "
          "the mouse to the image. Click and hold. Drag the mouse to select an "
  ...
  if (ImGui::CollapsingHeader("Mask Crop")) {
    ...
    if (showTooltip && ImGui::IsItemHovered()) {
      ImGui::SetTooltip("Override mask value validity check. But the size "
                        "still has to match the image.");
    }
    ...
    if (ImGui::Button("Load mask TIFF image")) {
      ...
      if (layerPerImg == 1) {
        ...
        if (maskPath == "")
          maskPath = FileDialog::openFileName(
              "./.*", {"*.tif", "*.tiff"}); // debug mode, pass in through '-m'
        if (!maskPath.empty()) {

          int layerPerImg_mask, channelPerSlice_mask, ttlFrames_mask;
          GetImageDescription(maskPath, layerPerImg_mask, channelPerSlice_mask,
                              ttlFrames_mask);

          if (layerPerImg_mask != layerPerImg) {
            logger().error(
                "Z-stack size in the mask does not match with the image");
            std::cerr
                << "Z-stack size in the mask does not match with the image"
                << std::endl;
          } else {
            if (ReadImageFirstFrame(maskPath, layerPerImg, 1, membraneMask,
                                    imageCrop.r0, imageCrop.c0, imageCrop.r1,
                                    imageCrop.c1, 0)) {
              if (imgRows == membraneMask[0].rows() &&
                  imgCols == membraneMask[0].cols()) {

                double minValue = std::numeric_limits<double>::max();
                double maxValue = std::numeric_limits<double>::min();
                for (int i = 0; i < membraneMask.size(); i++) {
                  if (membraneMask[i].maxCoeff() > maxValue)
                    maxValue = membraneMask[i].maxCoeff();
                  if (membraneMask[i].minCoeff() < minValue)
                    minValue = membraneMask[i].minCoeff();
                }
                bool ok = false;
                if (!overrideMaskCheck) {
                  if (minValue < 0 || maxValue > 255) {
                    logger().error("mask value not in [0, 255]: min={} max={}",
                                   minValue, maxValue);
                  } else if (maxValue < 128) {
                    logger().warn("mask max value < 128. Very likely to reject "
                                  "all markers.");
                  } else {
                    ok = true;
                  }
                ...
                logger().error("Row/Col number in each z-slice does not match "
                               "the image's number");
                std::cerr << "Row/Col number in each z-slice does not match "
                             "the image's number"
                          << std::endl;
              }
            } else {
    ...
    if (showTooltip && ImGui::IsItemHovered()) {
      ImGui::SetTooltip("[Optional] Load a TIFF image as a binary mask of the "
                        "membrane area.\nThe mask must be of the same "
                        "dimension as the background image.");
    }
    ImGui::SameLine();
  ...
  if (ImGui::CollapsingHeader("Microscope", ImGuiTreeNodeFlags_DefaultOpen)) {
    ...
    ImGui::PushItemWidth(UIsize.zebrafishWidth / 3.0);
    ImGui::InputDouble("Resolution X (um)", &resolutionX, 0.0f, 1000.0f,
                       "%.4f");
    ImGui::InputDouble("Resolution Y (um)", &resolutionY, 0.0f, 1000.0f,
                       "%.4f");
    ImGui::InputDouble("Resolution Z (um)", &resolutionZ, 0.0f, 1000.0f,
                       "%.4f");
    ImGui::PopItemWidth();
  }
  if (showTooltip && ImGui::IsItemHovered()) {
    ImGui::SetTooltip(
        "The physical distance of one pixel (in micrometers).\nResolution X is "
        "the distance between two rows; resolution Y is the distance between "
        "two columns; resolution Z is the vertical distance between two "
        "z-slices.");
  }
  ...
  if (ImGui::TreeNode("Advanced visualization")) {
      ...
      ImGui::PopItemWidth();
      if (showTooltip && ImGui::IsItemHovered()) {
          ImGui::SetTooltip("Enhance the contrast of the image. This will only
  take effect in stage one.");
      }
      ImGui::TreePop();
      ImGui::Separator();
  }
  */
  ...
  if (ImGui::Button("Apply")) {
    ...
    try {
      // NOTE: we crop the area [r0, c0]x[r1, c1] when loading the image, but
      // keep the entire z-stack despite the z-crop
      if (ReadImageFirstFrame(imagePath, layerPerImg, channelPerSlice,
                              imgData[0], imageCrop.r0, imageCrop.c0,
                              imageCrop.r1, imageCrop.c1, channelToLoad)) {
        ...
        NormalizeImage(
            img_, previewQuantileBrightness); // Do not modify "imgData[0]" now
      ...
      logger().error("   <button> [Apply] Fatal error when trying to re-load "
                     "the image. This is often due to wrong metadata like "
                     "incorrect number of channels or slices.");
      std::cerr << "   <button> [Apply] Fatal error when trying to re-load the "
                   "image. This is often due to wrong metadata like incorrect "
                   "number of channels or slices."
                << std::endl;
      applyStr = "Failed";
    }
  ...
  if (showTooltip && ImGui::IsItemHovered()) {
    ImGui::SetTooltip(
        "Preview the image with the parameters in this page.\nThe result will "
        "be the image used by upcoming stages.");
  }
  ImGui::SameLine();
...
void GUI::CropImage(const Eigen::Vector2f &mouse, MOUSE_TYPE mousetype,
  ...
  if (hit) {
    ...
    loc(0) = (V_texture(F_texture(fid, 0), 0) * bc(0) +
              V_texture(F_texture(fid, 1), 0) * bc(1) +
              V_texture(F_texture(fid, 2), 0) * bc(2));
    loc(1) = (V_texture(F_texture(fid, 0), 1) * bc(0) +
              V_texture(F_texture(fid, 1), 1) * bc(1) +
              V_texture(F_texture(fid, 2), 1) * bc(2));
    loc(2) = (V_texture(F_texture(fid, 0), 2) * bc(0) +
              V_texture(F_texture(fid, 1), 2) * bc(1) +
              V_texture(F_texture(fid, 2), 2) * bc(2));

    if (mousetype == MOUSEDOWN) {

      logger().debug("Mouse down: fid = {} | bc = {:.2f} x {:.2f} x {:.2f} | "
                     "loc = {:.2f} x {:.2f} x {:.2f}",
                     fid, bc(0), bc(1), bc(2), loc(0), loc(1), loc(2));
      cropStruct.downClicked = true;

      cropStruct.baseLoc = loc;
      cropStruct.currentLoc = loc;
    } else if (mousetype == MOUSEUP) {
      logger().debug("Mouse up:   fid = {} | bc = {:.2f} x {:.2f} x {:.2f} | "
                     "loc = {:.2f} x {:.2f} x {:.2f}",
                     fid, bc(0), bc(1), bc(2), loc(0), loc(1), loc(2));
      ...
      if (cropStruct.r0 > cropStruct.r1 && cropStruct.c0 > cropStruct.c1) {
        ...
        int t;
        t = cropStruct.c0;
        cropStruct.c0 = cropStruct.c1;
        cropStruct.c1 = t;
        t = cropStruct.r0;
        cropStruct.r0 = cropStruct.r1;
        cropStruct.r1 = t;
      }
...
void GUI::DrawRect(double x0, double y0, double x1, double y1,
                   const Eigen::MatrixXd &lineColor) {
  ...
  // upper-left corner (x0, y0)
  // lower-right corner (x1, y1)
  lineX << x0, y0, 0.1, x0, y1, 0.1, x1, y1, 0.1, x1, y0, 0.1;
  lineY << x0, y1, 0.1, x1, y1, 0.1, x1, y0, 0.1, x0, y0, 0.1;
