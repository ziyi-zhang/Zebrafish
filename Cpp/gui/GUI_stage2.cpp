...
void GUI::DrawStage2() {
  ...
  if (ImGui::CollapsingHeader("Preprocess", ImGuiTreeNodeFlags_DefaultOpen)) {
    ...
    ImGui::PlotHistogram("", imgHist.hist.data(), imgHist.hist.size(), 0, NULL,
                         0, imgHist.hist.maxCoeff(), ImVec2(0, 80));
    ImVec2 after = ImGui::GetCursorScreenPos();
    after.y -= ImGui::GetStyle().ItemSpacing.y;

    // logger().debug("normalizeQuantile = {}   normalizeQuantileRes = {}",
    // normalizeQuantile, normalizeQuantileRes);
    float ratio = ((normalizeQuantileRes - imgHist.minValue) /
                   (imgHist.maxValue - imgHist.minValue));
    ...
    if (ImGui::SliderFloat("", &normalizeQuantile, 0.95, 0.999, "%.3f") ||
        updateNormalizedTexture) {

      updateNormalizedTexture = false;
      normalizeQuantileRes =
          QuantileImage(imgData[0], normalizeQuantile, layerBegin, layerEnd);
      image_t img_ = imgData[0];
      NormalizeImage(img_,
                     normalizeQuantileRes); // Do not modify "imgData[0]" now
      logger().info(
          "Slider trial: normalizeQuantile =  {:.4f}  thres =  {:.4f}",
          normalizeQuantile, normalizeQuantileRes);
    ...
    if (showTooltip && ImGui::IsItemHovered()) {
      ImGui::SetTooltip(
          "Set the pixel brightness threshold. All pixels brighter than this "
          "value will be trimmed.\nAn appropriate value would move the green "
          "line at the tail of the histogram, leaving only negligible bars to "
          "its right.");
    }
    ImGui::PopItemWidth();
  }
  ...
  if (ImGui::CollapsingHeader("B-spline Config",
                              ImGuiTreeNodeFlags_DefaultOpen)) {

    if (ImGui::TreeNode("Advanced B-spline")) {

      if (ImGui::SliderInt("B-spline degree", &bsplineDegree, 2, 3,
                           "Degree %d")) {
        bsplineArray[0].Set_degree(bsplineDegree);
      }
      if (ImGui::InputDouble("B-spline solver tolerance", &bsplineSolverTol,
                             0.0, 0.0, "%.2E")) {
    ...
    if (ImGui::Button("Compute B-spline")) {
      ...
      // IMPORTANT: The effect of quantile does not affect the raw data until
      // now put into effect
      normalizeQuantileRes =
          QuantileImage(imgData[0], normalizeQuantile, layerBegin, layerEnd);
      ...
      logger().info("[Finalized] Image normalized with normalizeQuantile =  "
                    "{:.4f}  thres =  {:.4f}",
                    normalizeQuantile, normalizeQuantileRes);
      ...
      bsplineArray[0].CalcControlPts(imgData[0], bsp_xratio, bsp_yratio,
                                     bsp_zratio, bsplineDegree);
      bsplineStr = "Done";
      stage2Lock = true;
      logger().debug("   <button> Compute B-spline");
    }
    if (showTooltip && ImGui::IsItemHovered()) {
      ImGui::SetTooltip("Compute B-spline control points for the first frame. "
                        "This may take some time.\nNote: If the brightness "
                        "threshold changes, the B-spline must be re-computed.");
    }
    ImGui::SameLine();
  ...
  if (ImGui::Button("Interp Img")) {
      ...
      Eigen::VectorXd xArray = Eigen::VectorXd::LinSpaced(imgRows-4, 2,
  imgRows-3); Eigen::VectorXd yArray = Eigen::VectorXd::LinSpaced(imgCols-4, 2,
  imgCols-3);
      // Eigen::VectorXd zArray = Eigen::VectorXd::LinSpaced(layerPerImg-4,
  layerBegin+2, layerEnd-2); Eigen::VectorXd zArray =
  Eigen::VectorXd::LinSpaced(1, 4, 4);
          ...
          for (int iz=0; iz<zArray.size(); iz++) {
              ...
              for (int ix=0; ix<xArray.size(); ix++)
                  for (int iy=0; iy<yArray.size(); iy++) {
                      slice(ix, iy) = bsplineArray[0].Interp3D(xArray(ix),
  yArray(iy), zArray(iz));
                  }
              img.push_back(slice);
          }
      ...
      InterpImage();
      std::string outName = "/Users/ziyizhang/Projects/tmp/interp_zebra-" +
  std::to_string(bsp_xratio) + "-" + std::to_string(bsp_yratio) + ".tif";
  ...
  */
  ...
  if (ImGui::CollapsingHeader("Load All Frames",
                              ImGuiTreeNodeFlags_DefaultOpen)) {

    const float inputWidth = ImGui::GetWindowWidth() / 3.0;
    ImGui::PushItemWidth(inputWidth);

    ImGui::SliderInt("Desired #frames", &desiredFrames, 1, ttlFrames,
                     "%d frames");
    ...
    if (showTooltip && ImGui::IsItemHovered()) {
      ImGui::SetTooltip("[Optional] Load desired number of frames and prepare "
                        "them for optimization. This may take some time.\nThis "
                        "is optional as it can be postponed to stage-7.");
    }
...
void GUI::ComputeImgHist(const image_t &img_) {
  ...
  for (int i = layerBegin; i <= layerEnd; i++) {
    ...
    if (img_[i].maxCoeff() > maxValue)
      maxValue = img_[i].maxCoeff();
    if (img_[i].minCoeff() < minValue)
      minValue = img_[i].minCoeff();
  }
  ...
  for (int i = layerBegin; i <= layerEnd; i++) {
    ...
    for (int j = 0; j < mat.size(); j++) {
      tmp[std::floor((mat(j) - minValue) / gap)]++;
    }
  }

  imgHist.hist = tmp.cast<float>();
}
