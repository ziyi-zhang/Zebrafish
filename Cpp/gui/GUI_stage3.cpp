...
void GUI::DrawStage3() {
  ...
  if (showPromisingPoints) {
    ...
    if (promisingPointLoc.rows() > 0) {
      ...
      viewer.data().add_points(promisingPointLoc, pointColor);
    }
  ...
  if (ImGui::CollapsingHeader("Grid Search", ImGuiTreeNodeFlags_DefaultOpen)) {
    ...
    if (ImGui::TreeNode("Advanced search range")) {
      ...
      ImGui::InputDouble("Radius min (pixel)", &grid.rArrayMin, 0.0, 0.0,
                         "%.2f");
      ImGui::InputDouble("Radius max (pixel)", &grid.rArrayMax, 0.0, 0.0,
                         "%.2f");
      ImGui::InputDouble("Radius gap (pixel)", &grid.rArrayGap, 0.0, 0.0,
                         "%.2f");
      if (showTooltip && ImGui::IsItemHovered()) {
        ImGui::SetTooltip(
            "The following array of radii will be searched\n[R_min R_min+R_gap "
            "R_min+2*R_gap ... R_max]");
      }
      ...
      if (showTooltip && ImGui::IsItemHovered()) {
        ImGui::SetTooltip(
            "By default the markers should have darker color. If checked, the "
            "program will search for light color markers.");
      }
      ImGui::Checkbox("Skip membrane area check", &grid.skipMembrane);
      if (showTooltip && ImGui::IsItemHovered()) {
        ImGui::SetTooltip(
            "By default only cylinders in the membrane area will be used.");
      }
      ImGui::PopItemWidth();
    ...
    if (showTooltip && ImGui::IsItemHovered()) {
      ImGui::SetTooltip("Start grid search with the specified resolution. This "
                        "may take some time.");
    }
  }
  ...
  if (ImGui::CollapsingHeader("Grid Search Result",
    ...
    ImGui::PlotHistogram("", gridEnergyHist.hist.data(),
                         gridEnergyHist.hist.size(), 0, NULL, 0,
                         gridEnergyHist.hist.maxCoeff(), ImVec2(0, 80));
    ImVec2 after = ImGui::GetCursorScreenPos();
    after.y -= ImGui::GetStyle().ItemSpacing.y;

    float ratio = ((grid.energyThres - gridEnergyHist.minValue) /
                   (gridEnergyHist.maxValue - gridEnergyHist.minValue));
    ...
    if (ImGui::SliderFloat("", &grid.energyThres, gridEnergyHist.minValue,
                           gridEnergyHist.maxValue, "%.3f")) {

      UpdateSampleNewton(gridSampleInput, gridSampleOutput);
    }
    if (showTooltip && ImGui::IsItemHovered()) {
      ImGui::SetTooltip(
          "Set the grid search energy threshold. The next stage only optimizes "
          "points with smaller energy.\nA large threshold makes the detection "
    ...
    if (ImGui::TreeNode("Advanced visualization ")) {
      ...
      if (showTooltip && ImGui::IsItemHovered()) {
        ImGui::SetTooltip("After the energy filter, only these dots will be "
                          "used as optimization starting conditions.");
      }
...
void GUI::GridSearch() {
  ...
  const int Nr =
      std::round((grid.rArrayMax - grid.rArrayMin) / grid.rArrayGap + 1);

  Eigen::VectorXd rArray;
  rArray.resize(Nr);
  for (int i = 0; i < Nr; i++)
    rArray(i) = grid.rArrayMin + i * grid.rArrayGap;
  if (rArray(Nr - 1) > grid.rArrayMax)
    rArray(Nr - 1) = grid.rArrayMax;
    ...
    for (int iy = 0; iy < floor(Ny / grid.gapY); iy++)
      for (int iz = 0; iz < floor(Nz / grid.gapZ); iz++)
        for (int ir = 0; ir < Nr; ir++) {
          ...
          zz = iz * grid.gapZ;
          rr = rArray(ir);
          if (!ValidGridSearchPoint(imgData[0], bsplineArray[0],
                                    grid.skipMembrane, grid.membraneThres, xx,
                                    yy, zz, rr))
            continue;
  ...
  logger().debug("gapX = {:.2f}  gapY = {:.2f}  gapZ = {:.2f}", grid.gapX,
                 grid.gapY, grid.gapZ);
  ...
  tbb::parallel_for(
      tbb::blocked_range<int>(0, sampleCount),
      [this /*.bsplineArray[0], .gridSampleInput, .gridSampleOutput*/](
          const tbb::blocked_range<int> &r) {
        for (int ii = r.begin(); ii != r.end(); ++ii) {
          cylinder::EvaluateCylinder(
              bsplineArray[0], gridSampleInput(ii, 0), gridSampleInput(ii, 1),
              gridSampleInput(ii, 2), gridSampleInput(ii, 3), cylinder::H,
              gridSampleOutput(ii), invertColor);
        }
      });
  logger().info("<<<<<<<<<< After grid search <<<<<<<<<<");
}

bool GUI::InMembraneArea(const image_t &image, const double thres, double x,
  ...
  int count = 0;
  if (layer(int(x - d2), int(y - d2)) > thres)
    count++;
  if (layer(int(x - d1), int(y)) > thres)
    count++;
  if (layer(int(x - d2), int(y + d2)) > thres)
    count++;
  if (layer(int(x), int(y + d1)) > thres)
    count++;
  if (layer(int(x + d2), int(y + d2)) > thres)
    count++;
  if (layer(int(x + d1), int(y)) > thres)
...
bool GUI::ValidGridSearchPoint(const image_t &image, const bspline &bsp,
                               bool skipMembrane, double membraneThres,
                               double x, double y, double z, double r) {

  // valid cylinder
  if (!cylinder::IsValid(bsp, x, y, z, r, cylinder::H))
    return false;
  // whether in membrane area
  if (!skipMembrane)
    if (!InMembraneArea(image, membraneThres, x, y, z, r))
      return false;

  return true;
}

void GUI::UpdateSampleNewton(const Eigen::MatrixXd &gridSampleInput,
                             const Eigen::MatrixXd &gridSampleOutput) {
  /// This function will clear "pointRecord" and intialize it with grid search
  /// results

  // Note: gridSampleInput has a much larger reserved space, but the remaining
  // should be empty
  ...
  for (i = 0; i < N; i++)
    if (gridSampleOutput(i) < grid.energyThres)
      M++; // count to reserve space
...
void GUI::UpdatePromisingPointLoc() {
  ...
  promisingPointLoc.col(1) =
      (imgRows - 0.5) - pointRecord.grid_search.col(0).array();
  promisingPointLoc.col(2) = pointRecord.grid_search.col(2);

  logger().info("   [Visualization] Grid search promising points updated: "
                "total number = {}",
                N);
}
...
void GUI::UpdateGridEnergyHist() {
  ...
  tmp.setZero();
  for (int i = 0; i < N; i++) {
    tmp[std::floor((gridSampleOutput(i) - minValue) / gap)]++;
  }
  gridEnergyHist.hist = tmp.cast<float>();
}
