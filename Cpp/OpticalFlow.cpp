...
double GetMatValue(const std::vector<Eigen::MatrixXd> &mat, int x, int y,
                   int z) {
...
void Initialize3DMatrix(std::vector<Eigen::MatrixXd> &mat, int layer, int rows,
                        int cols) {
...
operator*(const std::vector<Eigen::MatrixXd> &matA,
          const std::vector<Eigen::MatrixXd> &matB) {
  // Elementwise product
...
void OpticalFlow::ComputePartialDerivative(
    const image_t &img1, const image_t &img2, PartialDerivative_t &dx,
    PartialDerivative_t &dy, PartialDerivative_t &dz, PartialDerivative_t &dt) {
  ...
  for (x = 0; x < rows; x++)
    for (y = 0; y < cols; y++)
      for (z = 0; z < layer; z++) {
        ...
        // consider an 8-element cube [x-1:x] * [y-1:y] * [z-1:z] for both img1
        // & img2
        dx[z](x, y) =
            (1.0 / 8.0) *
            (GetMatValue(img1, x, y, z) - GetMatValue(img1, x - 1, y, z) +
             GetMatValue(img1, x, y - 1, z) -
             GetMatValue(img1, x - 1, y - 1, z) +
             GetMatValue(img1, x, y, z - 1) -
             GetMatValue(img1, x - 1, y, z - 1) +
             GetMatValue(img1, x, y - 1, z - 1) -
             GetMatValue(img1, x - 1, y - 1, z - 1) +
             GetMatValue(img2, x, y, z) - GetMatValue(img2, x - 1, y, z) +
             GetMatValue(img2, x, y - 1, z) -
             GetMatValue(img2, x - 1, y - 1, z) +
             GetMatValue(img2, x, y, z - 1) -
             GetMatValue(img2, x - 1, y, z - 1) +
             GetMatValue(img2, x, y - 1, z - 1) -
             GetMatValue(img2, x - 1, y - 1, z - 1));

        dy[z](x, y) =
            (1.0 / 8.0) *
            (GetMatValue(img1, x, y, z) - GetMatValue(img1, x, y - 1, z) +
             GetMatValue(img1, x - 1, y, z) -
             GetMatValue(img1, x - 1, y - 1, z) +
             GetMatValue(img1, x, y, z - 1) -
             GetMatValue(img1, x, y - 1, z - 1) +
             GetMatValue(img1, x - 1, y, z - 1) -
             GetMatValue(img1, x, y - 1, z - 1) + GetMatValue(img2, x, y, z) -
             GetMatValue(img2, x, y - 1, z) + GetMatValue(img2, x - 1, y, z) -
             GetMatValue(img2, x - 1, y - 1, z) +
             GetMatValue(img2, x, y, z - 1) -
             GetMatValue(img2, x, y - 1, z - 1) +
             GetMatValue(img2, x - 1, y, z - 1) -
...
void OpticalFlow::ComputeAverageFlow(const FlowVelocity_t &u,
                                     FlowVelocity_t &u_bar) {
  ...
  for (x = 0; x < rows; x++)
    for (y = 0; y < cols; y++)
      for (z = 0; z < layer; z++) {
        ...
        u_bar[z](x, y) =
            (1.0 / 6.0) *
            (GetMatValue(u, x - 1, y, z) + GetMatValue(u, x + 1, y, z) +
             GetMatValue(u, x, y - 1, z) + GetMatValue(u, x, y + 1, z) +
             GetMatValue(u, x, y, z - 1) + GetMatValue(u, x, y, z + 1));
      }
}

void OpticalFlow::RunOpticalFlow(const image_t &img1, const image_t &img2,
                                 double alpha, int iter, FlowVelocity_t &ux,
                                 FlowVelocity_t &uy, FlowVelocity_t &uz) {
  ...
  for (i = 0; i < layer; i++)
    constMat[i] = Eigen::MatrixXd::Constant(
        rows, cols, alpha * alpha); // alpha^2 constant matrix
  ...
  for (i = 0; i < iter; i++) {
    ...
    // DEBUG PURPOSE
    // std::cout << ux[40](58, 25) << " " << uy[40](58, 25) << " " << uz[40](58,
    // 25) << std::endl;
  }
}
