...
template <typename T> class GetVal {
public:
  double operator()(const T x) const { return x.getValue(); }
};

template <> class GetVal<double> {
public:
...
void bspline::SetResolution(const double resX, const double resY,
                            const double resZ) {
...
void bspline::CalcControlPts_um(const image_t &image, const double distX,
                                const double distY, const double distZ,
                                const int degree_) {
  ...
  Nx = image[0].rows();
  Ny = image[0].cols();
  /// Note: Why the calculation of ratio for degree 2 & 3 B-spline are
  /// different?
...
void bspline::CalcControlPts(const image_t &image, const double xratio,
                             const double yratio, const double zratio,
                             const int degree_) {
  ...
  if (xratio < ratioThres || yratio < ratioThres || zratio < ratioThres)
    logger().warn(
        "Control point size too small: xratio={}, yratio={} and zratio={}.",
  ...
  numZ = ceil(Nz * zratio);
  num = numX * numY * numZ;
  logger().debug("Control points: num= {} numX= {} numY= {} numZ={}", num, numX,
                 numY, numZ);
  ...
  Atranspose.reserve(
      Eigen::VectorXi::Constant(Atranspose.cols(), (degree == 2 ? 27 : 64)));

  // the interval between two control points along a direction
  if (degree == 3) {
    gapX =
        double(Nx - 1) / double(numX - 1 - 2); // "-2" due to clamped B-spline
    gapY = double(Ny - 1) / double(numY - 1 - 2);
    gapZ = double(Nz - 1) / double(numZ - 1 - 2);
  } else if (degree == 2) {
    gapX =
        double(Nx - 1) / double(numX - 1 - 1); // "-1" due to clamped B-spline
    gapY = double(Ny - 1) / double(numY - 1 - 1);
    gapZ = double(Nz - 1) / double(numZ - 1 - 1);
  }
  logger().debug("xratio = {}  yratio = {}  zratio = {}", xratio, yratio,
                 zratio);
  logger().debug("gapX = {}px  gapY = {}px  gapZ = {}px", gapX, gapY, gapZ);
  logger().debug("gapX = {}um  gapY = {}um  gapZ = {}um", gapX * resolutionX,
                 gapY * resolutionY, gapZ * resolutionZ);
  ...
  for (auto it = image.begin(); it != image.end(); it++) {
    inputPts.segment(t * Nx * Ny, Nx * Ny)
        << Eigen::Map<const Eigen::VectorXd>(it->data(), Nx * Ny);
    t++;
  }
  ...
  /// This is useless because depth-axis will not be affected by [x, y, r] and
  /// thus has gradient zero
  // double version
...
void bspline::CalcLeastSquareMat(
    Eigen::SparseMatrix<double, Eigen::RowMajor> &A,
    Eigen::SparseMatrix<double, Eigen::ColMajor> &Atranspose) {
  ...
  for (iz = 0; iz <= Nz - 1; iz++)
    for (iy = 0; iy <= Ny - 1; iy++)
      for (ix = 0; ix <= Nx - 1; ix++) {
        ...
        if (refIdx_x == numX - 1 - (degree - 1))
          refIdx_x--;
        if (refIdx_y == numY - 1 - (degree - 1))
          refIdx_y--;
        if (refIdx_z == numZ - 1 - (degree - 1))
          refIdx_z--;
        ...
        //    logger().error("RowSum error: ix={}, iy={}, iz={}, rowSum={}", ix,
        //    iy, iz, rowSum);
        ///////////// DEBUG ONLY
      }
}

void bspline::SolveLeastSquare(
    const Eigen::SparseMatrix<double, Eigen::RowMajor> &A,
    const Eigen::SparseMatrix<double, Eigen::ColMajor> &Atranspose,
    const Eigen::VectorXd &inputPts) {
  ...
  case 3: {
    ...
    solver; A.makeCompressed(); logger().info("A compressed");
    solver.compute(A);
...
void bspline::CalcInitialGuess(const Eigen::VectorXd &inputPts,
                               Eigen::VectorXd &initialGuess) {
...
template <typename T>
void bspline::CalcBasisFunc(
    Eigen::Matrix<std::function<T(T)>, Eigen::Dynamic, Eigen::Dynamic,
                  Eigen::ColMajor> &basisT,
  ...
  if (degree == 3) { // cubic basis
    ...
    basisT(1, 0) = [gapT](T x) {
      x = x / gapT;
      return (7.0 / 4.0) * x * x * x - (9.0 / 2.0) * x * x + 3.0 * x;
    };
    basisT(0, 1) = [gapT](T x) {
      x = x / gapT;
      return -(1.0 / 4.0) * x * x * x + (3.0 / 2.0) * x * x - 3.0 * x + 2.0;
    };
    // cubic [11234]
    basisT(2, 0) = [gapT](T x) {
      x = x / gapT;
      return -(11.0 / 12.0) * x * x * x + (3.0 / 2.0) * x * x;
    };
    basisT(1, 1) = [gapT](T x) {
      x = x / gapT;
      return (7.0 / 12.0) * x * x * x - 3.0 * x * x + 9.0 / 2.0 * x -
             (3.0 / 2.0);
    };
    basisT(0, 2) = [gapT](T x) {
      x = x / gapT;
      return -(1.0 / 6.0) * x * x * x + (3.0 / 2.0) * x * x - (9.0 / 2.0) * x +
             (9.0 / 2.0);
    };
    // last 3 columns
    // cubic [11112]
    basisT(3, numT - 4) = [gapT, numT](T x) {
      x = numT - 3 - x / gapT;
      return -(x - 1.0) * (x - 1.0) * (x - 1.0);
    };
    // cubic [11123]
    basisT(2, numT - 4) = [gapT, numT](T x) {
      x = numT - 3 - x / gapT;
      return (7.0 / 4.0) * x * x * x - (9.0 / 2.0) * x * x + 3.0 * x;
    };
    basisT(3, numT - 5) = [gapT, numT](T x) {
      x = numT - 3 - x / gapT;
      return -(1.0 / 4.0) * x * x * x + (3.0 / 2.0) * x * x - 3.0 * x + 2.0;
    };
    // cubic [11234]
    basisT(1, numT - 4) = [gapT, numT](T x) {
      x = numT - 3 - x / gapT;
      return -(11.0 / 12.0) * x * x * x + (3.0 / 2.0) * x * x;
    };
    basisT(2, numT - 5) = [gapT, numT](T x) {
      x = numT - 3 - x / gapT;
      return (7.0 / 12.0) * x * x * x - 3.0 * x * x + 9.0 / 2.0 * x -
             (3.0 / 2.0);
    };
    basisT(3, numT - 6) = [gapT, numT](T x) {
      x = numT - 3 - x / gapT;
      return -(1.0 / 6.0) * x * x * x + (3.0 / 2.0) * x * x - (9.0 / 2.0) * x +
             (9.0 / 2.0);
    };
    ...
    for (int i = 2; i <= numT - 5; i++) {
      basisT(3, i - 2) = [gapT, i](T x) {
        x = x / gapT - i;
        return (1.0 / 6.0) * x * x * x + x * x + 2.0 * x + (4.0 / 3.0);
      };
      basisT(2, i - 1) = [gapT, i](T x) {
        x = x / gapT - i;
        return -(1.0 / 2.0) * x * x * x - x * x + (2.0 / 3.0);
      };
      basisT(1, i) = [gapT, i](T x) {
        x = x / gapT - i;
        return (1.0 / 2.0) * x * x * x - x * x + (2.0 / 3.0);
      };
      basisT(0, i + 1) = [gapT, i](T x) {
        x = x / gapT - i;
        return -(1.0 / 6.0) * x * x * x + x * x - 2.0 * x + (4.0 / 3.0);
      };
    }
    ...
    // last 2 columns
    // quadratic [1112]
    basisT(2, numT - 3) = [numT, gapT](T x) {
      x = numT - 2 - x / gapT;
      return (x - 1.0) * (x - 1.0);
    };
    // quadratic [1123]
    basisT(1, numT - 3) = [numT, gapT](T x) {
      x = numT - 2 - x / gapT - 1;
      return -(3.0 / 2.0) * x * x - x + (1.0 / 2.0);
    };
    basisT(2, numT - 4) = [numT, gapT](T x) {
      x = numT - 2 - x / gapT - 1;
      return (1.0 / 2.0) * x * x - x + (1.0 / 2.0);
    };
    // middle columns
    ...
    for (int i = 1; i <= numT - 4; i++) {
      ...
      basisT(0, i + 1) = [gapT, i](T x) {
        x = x / gapT - i;
        return (1.0 / 2.0) * x * x - 2.0 * x + 2.0;
      };
    }
  }
}
// explicit template instantiation
template void bspline::CalcBasisFunc(
    Eigen::Matrix<std::function<DScalar(DScalar)>, Eigen::Dynamic,
                  Eigen::Dynamic, Eigen::ColMajor> &basisT,
    const int numT, const double gapT);
template void bspline::CalcBasisFunc(
    Eigen::Matrix<std::function<double(double)>, Eigen::Dynamic, Eigen::Dynamic,
                  Eigen::ColMajor> &basisT,
    const int numT, const double gapT);
...
// explicit template instantiation
template DScalar bspline::Interp3D(const DScalar &x, const DScalar &y,
                                   const DScalar &z) const;
template double bspline::Interp3D(const double &x, const double &y,
                                  const double &z) const;

template <typename basisT, typename T>
void bspline::InterpDiskHelper(const T x, const T y, const double z, const T r,
                               Eigen::Matrix<T, Eigen::Dynamic, 1> &res,
                               const basisT &basisX_, const basisT &basisY_,
                               const basisd_t &basisZ_) const {
  ...
  for (i = 0; i < quad.numDiskPts; i++) {
    ...
    xT = quad.xyArray(i, 0) * r + x, yT = quad.xyArray(i, 1) * r + y;
    // Get reference index
    ...
    /// Uncomment the following three lines to fix this at the cost of ~5% more
    /// time
    ...
    assert(refIdx_x <= numX - (degree + 1) && refIdx_y <= numY - (degree + 1) &&
           refIdx_z <= numZ - (degree + 1));
    ...
    for (iz = 0; iz <= degree; iz++)
      for (ix = 0; ix <= degree; ix++)
        for (iy = 0; iy <= degree; iy++) {
          ...
          res(i) += controlPoints(numX * numY * (refIdx_z + iz) +
                                  numY * (refIdx_x + ix) + (refIdx_y + iy)) *
                    basisX_v[ix] * basisY_v[iy] * basisZ_v[iz];
        }
  }
}
// explicit template instantiation
template void bspline::InterpDiskHelper(
    const DScalar x, const DScalar y, const double z, const DScalar r,
    Eigen::Matrix<DScalar, Eigen::Dynamic, 1> &res, const basis_t &basisX_,
    const basis_t &basisY_, const basisd_t &basisZ_) const;
template void bspline::InterpDiskHelper(
    const double x, const double y, const double z, const double r,
    Eigen::Matrix<double, Eigen::Dynamic, 1> &res, const basisd_t &basisX_,
    const basisd_t &basisY_, const basisd_t &basisZ_) const;

void bspline::InterpDisk(const DScalar x, const DScalar y, const double z,
                         const DScalar r,
                         Eigen::Matrix<DScalar, Eigen::Dynamic, 1> &res) const {
  // NOTE: This interpolation function cannot query points lying exactly on the
  // surface
...
void bspline::InterpDisk(const double x, const double y, const double z,
                         const double r,
                         Eigen::Matrix<double, Eigen::Dynamic, 1> &res) const {
  // NOTE: This interpolation function cannot query points lying exactly on the
  // surface
  //       of the 3D sample grid.

  InterpDiskHelper<basisd_t, double>(x, y, z, r, res, basisXd, basisYd,
                                     basisZd);
